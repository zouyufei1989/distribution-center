...
public class UpYun {
    final static Logger logger = LoggerFactory.getLogger(UpYun.class);
    /**
     * 默认的编码格式
     */
     ...
     * @throws IOException
     */
    public boolean writeFileIcon(String filePath, File file, boolean auto)
            throws IOException {
        return writeFileIcon(filePath, file, auto, null);
    }
    ...
    public boolean writeFile(String filePath, File file, boolean auto, Map<String, String> params) throws IOException {
        ...
        try {
            ...
            URL url = new URL("http://" + apiDomain + filePath);
            conn = (HttpURLConnection) url.openConnection();
            logger.info("upyun upload path:" + url);
